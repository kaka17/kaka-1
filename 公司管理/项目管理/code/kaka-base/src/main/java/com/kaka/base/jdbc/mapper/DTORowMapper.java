...
public class DTORowMapper<T> implements RowMapper<T> {
	...
	// 设置需要返回的对象的类型
	public DTORowMapper(Class<T> cls) {
		this.cls = cls;
	}
	...
	public T mapRow(ResultSet rset, int paramInt) throws SQLException {
		try {
			...
			for (int i = 0; i < columnCount; i++) {
				...
				for (Field fd : fields) {
					if (!cn.toLowerCase().endsWith(fd.getName().toLowerCase())) {
						continue;
					}
